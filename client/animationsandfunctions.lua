...
function ProgressBar(text, delay, event, skillcheck, failMessage, failEvent)
	exports["rpemotes"]:EmoteCommandStart("uncuff", 0)
    QBCore.Functions.Progressbar("drink_something", text, delay, false, true, {
		...
		if event then
			if skillcheck then
				local success = lib.skillCheck({'easy', 'medium', 'easy'}, {'1', '2', '3', '4'})
				if success then
					TriggerServerEvent(event)
				else
					if failEvent then
						TriggerServerEvent(failEvent)
					end
					if failMessage then
						QBCore.Functions.Notify(failMessage, 'error')
					end
				end
			else
				TriggerServerEvent(event)
