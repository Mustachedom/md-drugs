...
function Cornersell()
    ...
    local item, amount = lib.callback.await('md-drugs:server:cornerselling:getAvailableDrugs', false)
        if item == 'nothing' then Notify(' You Aint Got Shit', 'error') sell = false  FreezeEntityPosition(targ, false)  ClearPedTasks(targ) return false end
            ...
            local options = {
                { label = 'Sell ' .. bagAmount .. ' Of ' .. GetLabel(item) .. ' For $' .. price .. '!', icon = "fas fa-eye", 
                    onSelect = function() 
                        PoliceCall(20)
                        targbusy = true
