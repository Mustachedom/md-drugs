...
local function RequestDelivery()
    if not waitingDelivery then
        ...
        if Config.NearbyDeliveries then
        ...
        QBCore.Functions.Notify("You haven't even finished the one I told you to.", 'error')
    end
end
...
function InitZones()
    if next(QBConfig.Dealers) == nil then
        return
    end
